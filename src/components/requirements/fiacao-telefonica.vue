<template>
    <Question>
        O ambiente possui fiação telefônica
        <IonIcon v-if="isChecked" id="checkRequirements" :icon="checkmarkDoneSharp"></IonIcon>
        <template v-slot:input>
            <radioQuestion v-model="model[0]" id="fiacao-telefonica"></radioQuestion>
        </template>
        <template v-slot:sub>
            <template v-if="model[0]">
                <div class="question">
                    <p class="statement">A fiação telefônica do ambiente é embutida ou recoberta</p>
                    <radioQuestion v-model="model[1]" id="fiacao-embutida-telefonica"></radioQuestion>
                </div>
            </template>
        </template>
    </Question>
