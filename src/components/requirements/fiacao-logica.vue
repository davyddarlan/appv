<template>
    <Question>
        O ambiente possui fiação lógica
        <template v-slot:input>
            <radioQuestion v-model="model[0]" id="fiacao-logica"></radioQuestion>
        </template>
        <template v-slot:sub>
            <template v-if="model[0]">
                <div class="question">
                    <p class="statement">A fiação lógica do ambiente é embutida ou recoberta</p>
                    <radioQuestion v-model="model[1]" id="fiacao-embutida"></radioQuestion>
                </div>
            </template>
        </template>
    </Question>
